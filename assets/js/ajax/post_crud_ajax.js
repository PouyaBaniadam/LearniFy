...
function enableEditing(postId) {
    ...
    textarea.classList.add('form-textarea', 'w-full', '!ring-0', '!ring-offset-0', 'bg-secondary', 'border-0', 'focus:border-border', 'border-border', 'rounded-xl', 'text-sm', 'text-foreground', 'p-5');
...
function saveChanges(post_id) {
    ...
    const data = {
        caption: editedCaption, post_id: postId
    };

    fetch(`/account/profile/post/caption/update/`, {
        method: 'POST', headers: {
            'Content-Type': 'application/json', 'X-CSRFToken': getCookie('csrftoken') // Use a function to get CSRF token
...
function showAddPostForm() {
    Swal.fire({
        ...
        html: '<input id="imageInput" type="file" accept="image/*" required>' + '<input id="titleInput" maxlength="50" type="text" class="form-textarea w-full form-textareaw-full !ring-0 !ring-offset-0 bg-secondary border-0 focus:border-border border-border rounded-xl text-sm text-foreground p-5!ring-0 mb-3 mt-3" placeholder="موضوع" required>' + '<textarea id="captionInput" maxlength="1000" class="h-full form-textarea w-full form-textareaw-full !ring-0 !ring-offset-0 bg-secondary border-0 focus:border-border border-border rounded-xl text-sm text-foreground p-5!ring-0" placeholder="کپشن"></textarea>',
        focusConfirm: false,
        preConfirm: () => {
            ...
            return fetch('/account/profile/post/add/', {
                ...
                .then(data => {
                    if (data && data.error) {
                        ...
                        Swal.fire({
                            icon: 'success', title: 'پست با موفقیت افزوده شد.', confirmButtonText: 'باشه',
                        }).then(() => {
                            location.reload();
                        });
                    }
